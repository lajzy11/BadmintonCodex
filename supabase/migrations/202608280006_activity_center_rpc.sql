-- Returns the current owner's activity-center projection in one authorized
-- query. This avoids fragile client-side joins and only exposes summary fields.
...
begin
  ...
  select coalesce(
    jsonb_agg(
      ...
      ) order by a.activity_date, a.scheduled_start_at nulls last
    ),
    '[]'::jsonb
  ) into activity_items
