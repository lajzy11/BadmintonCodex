-- Keep navigation identity and registration limits consistent with PRD v2.4.
alter table public.profiles
  add constraint profiles_display_name_max_length check (char_length(display_name) <= 40);

alter table public.organizations
  add constraint organizations_name_max_length check (char_length(name) <= 40);
...
begin
  ...
  select p.username_normalized, p.display_name
  into account_name_value, display_name_value
