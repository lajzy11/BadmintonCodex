...
function MatchHistoryPanel({ activityId, onClose }: { activityId: string; onClose: () => void }) {
  ...
  const [pendingCancel, setPendingCancel] = useState<MatchHistoryItem | null>(null)
  ...
  async function cancelRecord(match: MatchHistoryItem) {
    try { await cancelCompleted.mutateAsync(match.id); if (editing?.id === match.id) setEditing(null); setPendingCancel(null); setMessage('對戰紀錄已取消，相關統計已更新。') }
  ...
  return <><div className="panel-backdrop" role="presentation"><section className="match-history-panel" role="dialog" aria-modal="true" aria-labelledby="match-history-title">
    ...
    {history.isLoading ? <p className="muted">載入紀錄…</p> : history.isError ? <p>目前無法載入對戰紀錄。</p> : <div className="match-history-list">{history.data?.map((match) => <article className={match.status === 'cancelled' ? 'match-record cancelled' : 'match-record'} key={match.id}>
      ...
      <div>{match.status === 'cancelled' ? <span className="muted">已取消</span> : <><button className="secondary-button" onClick={() => setEditing(match)}>補登／修改</button><button className="text-button danger-text" disabled={cancelCompleted.isPending} onClick={() => setPendingCancel(match)}>取消紀錄</button></>}</div>
  ...
  </section></div>{pendingCancel && <ConfirmDialog title={`取消「${pendingCancel.court_name}」的完成紀錄？`} confirmLabel="取消紀錄" pending={cancelCompleted.isPending} onCancel={() => setPendingCancel(null)} onConfirm={() => void cancelRecord(pendingCancel)}><p>此操作不可復原，場次與排點統計會重新計算。</p></ConfirmDialog>}</>
}
...
export function DispatchBoard({ activityId }: { activityId: string }) {
  ...
  const [pendingCancelMatch, setPendingCancelMatch] = useState<{ id: string; courtName: string } | null>(null)
  ...
  function cancel(matchId: string) { const court = state.courts.find((item) => item.active_match_id === matchId); setPendingCancelMatch({ id: matchId, courtName: court?.name ?? '這面球場' }) }
  async function confirmCancel() { if (!pendingCancelMatch) return; try { await cancelMatch.mutateAsync(pendingCancelMatch.id); setPendingCancelMatch(null); setMessage('本場已取消。') } catch { setMessage('取消失敗，資料可能已更新。') } }
  ...
  return <div className="dispatch-board" data-mobile-section={mobileSection}><div className="dispatch-control-bar">{state.assign_mode === 'system_assign' && <button type="button" role="switch" aria-checked={autoStatus.data?.auto_mode_enabled ?? false} className={`auto-mode-switch ${autoStatus.data?.auto_mode_enabled ? 'active' : ''}`} disabled={setAutoMode.isPending || state.status !== 'in_progress'} onClick={toggleAutoMode}><span aria-hidden="true"><i /></span>自動模式</button>}<button className="text-button" disabled={state.assign_mode !== 'system_assign'} onClick={() => setShowSettings(true)}>排點設定</button><button className="text-button" onClick={() => setShowHistory(true)}>對戰紀錄</button></div><nav className="dispatch-mobile-tabs" aria-label="排點內容"><button className={mobileSection === 'venues' ? 'active' : ''} onClick={() => setMobileSection('venues')}>場地</button><button className={mobileSection === 'players' ? 'active' : ''} onClick={() => setMobileSection('players')}>球友</button></nav>{state.status !== 'in_progress' && <div className="validation-box"><p>{state.status === 'ended' ? '活動已結束；可查看對戰紀錄，但不能再新增預排或開始新比賽。' : '活動尚未到最早方案時間；目前可以檢視配置，但不能加入預排或安排上場。'}</p></div>}{message && <p className="action-message" role="status">{message}</p>}
    ...
    <section className="dispatch-section court-console-section">
      <header><div><h2>球場與預排</h2><p className="muted">結束本場後可到對戰紀錄補登比分。</p></div><div className="resource-controls"><strong>管理球場 · {state.courts.length} 面</strong><button aria-label="新增球場" onClick={() => changeResource('court', 'add')}>＋</button></div></header>
      <div className="dispatch-card-grid">{state.courts.map((court, courtIndex) => {
        const teamA = court.players.filter((player) => player.team === 'A')
        const teamB = court.players.filter((player) => player.team === 'B')
        return <article className={`court-card court-console ${court.status}`} key={court.id}>
          <header className="court-console-header"><strong>{String(courtIndex + 1).padStart(2, '0')}</strong>{court.status === 'playing' ? <time>{elapsedTime(court.started_at, clock)}</time> : <time>—</time>}<span>{court.status === 'playing' ? '比賽中' : '空場'}</span></header>
          <div className={`court-player-layout ${court.status === 'idle' ? 'empty' : ''}`}><div className="court-team-row">{[0,1].map((position) => { const player = teamA[position]; return <div className="court-player" key={`A-${position}`}>{player ? <><strong>{player.display_name}</strong><small>Lv.{player.level}</small></> : <><strong>空位</strong><small>尚未安排</small></>}</div> })}</div><div className="court-versus"><span>VS</span></div><div className="court-team-row">{[0,1].map((position) => { const player = teamB[position]; return <div className="court-player" key={`B-${position}`}>{player ? <><strong>{player.display_name}</strong><small>Lv.{player.level}</small></> : <><strong>空位</strong><small>尚未安排</small></>}</div> })}</div></div>
          <footer className="court-console-actions">
            <details className="court-more-menu court-settings-menu"><summary aria-label={`設定${court.name}`} title="設定"><MaterialIcon name="settings" /></summary><div><button onClick={() => setRenameCourt({ id: court.id, name: court.name })}>編輯名稱</button>{court.status === 'playing' && <button onClick={() => cancel(court.active_match_id!)}>取消本場</button>}<button className="danger-text" disabled={court.status !== 'idle' || state.courts.length <= 1} onClick={() => setDeleteCourt({ id: court.id, name: court.name })}>刪除空場</button></div></details>
            <button className="voice-call-button" aria-label={`再次叫號 ${court.name}`} title="再次叫號" disabled={court.status !== 'playing' || autoStatus.data?.tts_settings?.enabled === false} onClick={() => callCourt(court.name, court.players)}><MaterialIcon name="volumeUp" /></button>
            <button className="primary-button" disabled={court.status !== 'playing'} onClick={() => court.active_match_id && finish(court.active_match_id)}>結束本局</button>
          </footer>
        </article>
      })}</div>
    </section>
    <section className="dispatch-section preview-console-section">
      <header><div><h2>預排順序</h2><p className="muted">預排接續場地工作流；可移出球友或處理時間到期。</p></div><div className="resource-controls"><button onClick={() => changeResource('queue', 'remove')}>−</button><strong>{state.queues.length}</strong><button onClick={() => changeResource('queue', 'add')}>＋ 新增預排</button></div></header>
      <div className="dispatch-card-grid">{state.queues.map((queue) => <article className="queue-card" key={queue.id}>
        <div className="dispatch-card-title"><h3><small className="queue-next">NEXT</small> 預排 {queue.sort_order + 1}</h3><span>{queue.members.length} / 4 人</span></div>
        <div className="preview-slots">{[1,2,3,4].map((slot) => { const member = queue.members.find((item) => item.slot === slot); const issue = member ? previewIssues.data?.find((item) => item.queue_id === queue.id && item.member_id === member.member_id) : undefined; return <Fragment key={slot}><div className={member ? issue?.expired ? 'filled expired' : 'filled' : ''}>{member ? <><strong>{member.display_name}</strong><small>Lv.{member.level}</small>{issue?.expired && <small className="expiry-label">時間已到{issue.one_time_expired_override ? '・本次允許' : ''}</small>}<div className="preview-slot-actions">{issue?.expired && !issue.one_time_expired_override && <button onClick={() => manageQueuedMember(queue.id, member.member_id, 'allow_expired_once')}>本次允許</button>}<button onClick={() => manageQueuedMember(queue.id, member.member_id, 'remove')}>移出</button></div></> : <span>空位</span>}</div>{slot === 2 && <div className="preview-versus" aria-hidden="true"><span>VS</span></div>}</Fragment> })}</div>
        <div className="deploy-row"><button className="secondary-button" disabled={fillAutoPreviews.isPending || state.assign_mode !== 'system_assign'} onClick={smartFill}><MaterialIcon name="refresh" />重算預排</button><button className="primary-button" disabled={queue.members.length !== 4 || idleCourts.length === 0} onClick={() => deploy(queue.id)}>一鍵上場</button></div>
      </article>)}</div>
    </section>
    ...
    {pendingCancelMatch && <ConfirmDialog title={`取消「${pendingCancelMatch.courtName}」進行中的對戰？`} confirmLabel="取消本場" pending={cancelMatch.isPending} onCancel={() => setPendingCancelMatch(null)} onConfirm={() => void confirmCancel()}><p>球員會回到可排點狀態，且本場不列入有效紀錄。</p></ConfirmDialog>}
  </div>
}
